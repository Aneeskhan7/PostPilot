...
-- Creates all 5 tables, RLS policies, triggers
-- Run this on a fresh Supabase project (Week 1 setup)
...
CREATE POLICY "Users see own accounts" ON social_accounts FOR SELECT USING (auth.uid() = user_id);
...
CREATE INDEX posts_status_scheduled_at ON posts(status, scheduled_at) WHERE status = 'scheduled';
...
CREATE POLICY "Users see own analytics" ON post_analytics FOR SELECT
...
CREATE POLICY "Users see own jobs" ON scheduled_jobs FOR SELECT
  USING (auth.uid() = (SELECT user_id FROM posts WHERE id = post_id));
CREATE TRIGGER set_updated_at BEFORE UPDATE ON scheduled_jobs FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE TRIGGER reset_monthly_count
