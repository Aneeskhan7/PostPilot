...
const AdminUserDetail: FC = () => {
  ...
  return (
    <div className="p-4 lg:p-8 space-y-5 lg:space-y-6 max-w-3xl">
      <Link
        to="/admin/users"
      ...
      {/* Profile card */}
      <div className="bg-white/5 border border-white/10 rounded-xl p-5 lg:p-6 space-y-4">
        {/* Header row */}
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0 ${PLAN_LABELS[user.plan].color}`}>
        ...
        <div className="grid grid-cols-3 gap-3 pt-4 border-t border-white/5">
          {[
            { label: 'Posts today',      value: user.daily_post_count   },
            { label: 'Posts this month', value: user.monthly_post_count },
            { label: 'Total posts',      value: user.total_posts         },
          ].map(({ label, value }) => (
            <div key={label} className="bg-white/5 rounded-xl p-3 text-center">
              <p className="text-xs text-zinc-500 mb-1 leading-snug">{label}</p>
        ...
        <div className="pt-4 border-t border-white/5">
          <p className="text-xs font-medium text-zinc-400 mb-2">Change plan</p>
          <div className="flex items-center gap-2 flex-wrap">
            <select
              value={currentPlan}
              onChange={(e) => setSelectedPlan(e.target.value as Plan)}
              className="px-3 py-2 bg-zinc-900 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-violet-500/50 transition"
      ...
      <div>
        <h2 className="text-sm font-semibold text-white mb-3">
          Connected Accounts ({user.social_accounts.length})
        </h2>
        {user.social_accounts.length === 0 ? (
          ...
          <div className="space-y-2">
            {user.social_accounts.map((acc) => {
              ...
              const expiring = acc.token_expires_at &&
                new Date(acc.token_expires_at) < new Date(Date.now() + 10 * 24 * 60 * 60 * 1000);
              return (
                <div key={acc.id} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {acc.page_name && <p className="text-xs text-zinc-500 truncate">{acc.page_name}</p>}
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {expiring && (
                      <span className="hidden sm:inline text-xs text-amber-400">Expiring</span>
                    )}
